...
<html>
   ...
   int count=1;
   out.println("<form action='postadd1.jsp' method='post'><center><table class='tsc_table_s9' summary='Sample Table' style='width:80%;'><thead><tr><th>S No.</th><th>Year</th><th>Subject</th><th>Section</th></tr></thead><tfoot><tr><th scope='row'>Total</th><td colspan='7'>4 buildings</td></tr></tfoot><tbody>");
   while(rs.next())
      {String tablename=rs.getString(1).charAt(0)+rs.getString(3);
           out.println("<tr class='odd'><td>"+count+"</td><td><input style='display:none;' name='year' value='"+rs.getString(1)+"'>"+rs.getString(1)+"</td>"+"</td><td><input type='hidden' name='sub' value='"+rs.getString(2)+"'>"+rs.getString(2)+"</td>"+"</td><td><input type='hidden' name='sec1' value='"+rs.getString(3)+"'>"+"<input type='radio' id='radio"+count+"' name='sec' value='"+tablename+"'><label for ='radio"+count+"'>"+rs.getString(3)+"</label></input></td><tr>"); 
      count++;
      }
